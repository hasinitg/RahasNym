...
public class ZKP_NI {
    ...
    //output file:
    private static String RESULT_FILE_NAME = "./Results_ZKP_NI.txt";
    ...
    public static void main(String[] args) throws CryptoAlgorithmException, NoSuchAlgorithmException, InvalidKeySpecException {
        try {
            ...
            List<BigInteger> challenges = measureTimeForChallengeCreation(originalCommitments, helperCommitments);
            ...
            measureTimeForProofVerification(originalCommitments, helperCommitments, challenges, proofs);
    ...
    private static List<BigInteger> measureTimeForChallengeCreation(List<PedersenCommitment> originalCommitments,
                                                                    List<PedersenCommitment> helperCommitments) throws NoSuchAlgorithmException {
        ...
        for (int l = 0; l < DATA_SET_SIZE; l++) {
            ...
            Long start = System.nanoTime();
            List<BigInteger> createdChallenges = zkpClient.createChallengeForNonInteractiveZKP(originalCommitment, randomCommitments);
            Long end = System.nanoTime();
    ...
    private static List<PedersenCommitmentProof> measureTimeForProofCreation(
        ...
        for (int p = 0; p < DATA_SET_SIZE; p++) {
            ...
            BigInteger secretCommittable = CryptoUtil.getCommittableThruPBKDF(password, currentSalt, publicParams.getQ().bitLength() - 1, 1000);
    ...
    private static void measureTimeForProofVerification(List<PedersenCommitment> originalCommitments,
                                                        List<PedersenCommitment> helperCommitments, List<BigInteger> challenges,
        ...
        for (int s = 0; s < DATA_SET_SIZE; s++) {
            ...
            Long start = System.nanoTime();
            boolean success = zkpServer.verifyNonInteractiveZKP(originalCommitment, currentHelperCommitments,
                    currentChallenges, currentProofs);
            Long end = System.nanoTime();
            Long elapsed = end - start;
            elapsedTimes.add(elapsed);
            if (!success) {
